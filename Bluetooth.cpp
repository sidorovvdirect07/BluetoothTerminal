...
void Bluetooth::serviceDiscovered(QBluetoothUuid uuid)
{
    ...
    if(uuid == UARTuuid){
        ...
        if(service){
            ...
            connect(service,QOverload<QLowEnergyService::ServiceError>::of(&QLowEnergyService::error),this,[&](QLowEnergyService::ServiceError err){
                qDebug()<<"ERROR"<<err;
            });
...
void Bluetooth::handleServiceStateChange(QLowEnergyService::ServiceState state)
{
    switch(state){
        ...
        //Scope needed for inner variables
        {
            ...
            auto desc=rxChar.descriptor(QBluetoothUuid::fromString(QString("{00002902-0000-1000-8000-00805f9b34fb}")));
            if(desc.isValid()){
